...
public class RoomsController extends MainController {
    ...
    private String sqlCountFreeRooms = "SELECT * FROM rooms WHERE status = 'Frei'";
    private String sqlCountBookedRooms = "SELECT * FROM rooms WHERE status = 'Belegt'";
    ...
    // columnRoomNumber.setCellFactory(TextFieldTableCell.forTableColumn(new
    // LongStringConverter()));
    ...
    public void loadRoomsData() {
        ...
        try {
            ...
            while (rs.next()) {
                this.roomList.add(new RoomsData(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4),
    ...
    public int getFreeRoomsNumber(){
        int count = 0;
        try {

            Connection conn = dbConnection.getConnection();

            ResultSet rs = conn.createStatement().executeQuery(sqlCountFreeRooms);
            while (rs.next()) {
                rs.getInt(1);
                count++;
            }
    ...
    public int getBookedRoomsNumber(){
        int count = 0;
        try {

            Connection conn = dbConnection.getConnection();

            ResultSet rs = conn.createStatement().executeQuery(sqlCountBookedRooms);
            while (rs.next()) {
                rs.getInt(1);
                count++;
            }
